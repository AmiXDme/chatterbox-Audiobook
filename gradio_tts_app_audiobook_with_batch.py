...
def generate(model, text, audio_prompt_path, exaggeration, temperature, seed_num, cfgw, language_id="en", progress=gr.Progress(track_tqdm=True)):
    if model is None:
        model = ChatterboxTTS.from_pretrained(DEVICE)
...
def generate_with_retry(model, text, audio_prompt_path, exaggeration, temperature, cfg_weight, max_retries=3, language_id="en"):
...
def save_voice_profile(voice_library_path, voice_name, display_name, description, audio_file, exaggeration, cfg_weight, temperature, enable_normalization=False, target_level_db=-18.0, language_id="en"):
...
def create_audiobook_with_volume_settings(model, text_content, voice_library_path, selected_voice, project_name, 
    ...
    if voice_config:
        ...
        result = create_audiobook(model, text_content, voice_library_path, temp_voice_name, project_name, language_id=language_id, progress=progress)
...
def create_multi_voice_audiobook_with_volume_settings(model, text_content, voice_library_path, project_name, 
    ...
    if enable_norm:
        ...
        result = create_multi_voice_audiobook_with_assignments(
            model, text_content, voice_library_path, project_name, temp_assignments, language_id=language_id, progress=progress
        )
        ...
        return create_multi_voice_audiobook_with_assignments(
            model, text_content, voice_library_path, project_name, voice_assignments, language_id=language_id, progress=progress
        )
...
with gr.Blocks(css=css, title="Chatterbox TTS - Audiobook Edition") as demo:
    ...
    save_voice_btn.click(
        fn=lambda path, name, display, desc, audio, exag, cfg, temp, enable_norm, target_level, lang: save_voice_profile(
            path, name, display, desc, audio, exag, cfg, temp, enable_norm, target_level, language_id=lang
        ),
        inputs=[
    ...
    regenerate_current_btn.click(
        ...
        inputs=[model_state, listen_edit_project_name, current_chunk_state, current_chunk_text, voice_library_path_state, studio_language],
