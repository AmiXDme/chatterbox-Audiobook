...
class AttentionQKV(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        # q: (b, h, t, d), k: (b, h, s, d), v: (b, h, s, d) -- cross-attention where t != s
        sim = torch.einsum("bhtd,bhsd->bhts", q, k) * self.scale
        if mask is not None:
...
class AttentionBlock2(nn.Module):
    ...
    def __init__(
        ...
        self.attention = AttentionQKV(self.num_heads, channels // self.num_heads, dropout_rate=dropout_rate, flash=False, scale=scale)
